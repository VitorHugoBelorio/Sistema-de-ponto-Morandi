...
$queryVerificar = "SELECT * FROM funcionario WHERE funcionario_cpf = '$cpf' OR nome = '$nomeCompleto'";
$resultadoVerificar = mysqli_query($conexao, $queryVerificar);

if (mysqli_num_rows($resultadoVerificar) > 0) {
    // Se o CPF ou nome já existirem, exibe uma mensagem de erro
    $_SESSION['mensagem'] = "Já existe um funcionário cadastrado com este CPF ou Nome.";
    header('Location: ../indexCadastrarFuncionario.php');
    exit();
}
...
$queryFuncionario = "INSERT INTO funcionario (funcionario_cpf, nome, senha, email, data_do_registro, cargo) 
                     VALUES ('$cpf', '$nomeCompleto', MD5('$senha'), '$email', NOW(), 'FUNCIONARIO')";
...
if (mysqli_query($conexao, $queryFuncionario)) {
    ...
    $queryGeral = "INSERT INTO geral (id_funcionario, cpf, nome, senha, email, data_do_registro, cargo) 
                   VALUES ('$idFuncionario', '$cpf', '$nomeCompleto', MD5('$senha'), '$email', NOW(), 'FUNCIONARIO')";
